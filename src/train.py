...
def train_model(yaml_config_path: Path, load_model_path: Path = None) -> Path:
    """
    Train AI using config file, with optional checkpoint loading for curriculum learning.
    
    :param yaml_config_path: The config file path
    :param load_model_path: Path to an existing .zip model to continue training from
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix=f"{run_name}_step"
    )

    if load_model_path and load_model_path.exists():
        if config['algo'] == 'SAC':
            model = SAC.load(load_model_path, env=env, tensorboard_log=TENSORBOARD_DIR)
        elif config['algo'] == 'PPO':
            model = PPO.load(load_model_path, env=env, tensorboard_log=TENSORBOARD_DIR)
        elif config['algo'] == 'TD3':
            model = TD3.load(load_model_path, env=env, tensorboard_log=TENSORBOARD_DIR)
        
        model.learning_rate = hp['learning_rate']
        model.ent_coef = hp.get('ent_coef', 'auto')
    else:
        if config['algo'] == 'PPO':
            model = PPO(
                config['policy'],
    ...
    path = MODELS_DIR / run_name / config['meta']['run_name']
